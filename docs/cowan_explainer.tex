...
The number of radial nodes ($n-\ell-1$) and the outward march of the orbitals
with $n$ are exactly what determine the sizes of the radial integrals below.

\begin{figure}[h]
\centering
\includegraphics[width=0.74\textwidth]{orbitals.pdf}
\caption{Reduced radial wavefunctions $P_{n\ell}(r)=r\,R_{n\ell}(r)$.
\emph{(Illustrative: exact hydrogenic functions, $Z=1$; a real \prog{rcn} run
produces the self-consistent-field analogues for the actual ion.)} Note the
nodal structure --- $1s,2p,3d$ are nodeless, $2s,3p$ have one node --- and that
higher-$n$ orbitals peak at larger radius.}
\label{fig:orbitals}
\end{figure}
...
each output file together, annotating the real column formats --- and replace the
illustrative figures here with the actual computed $P_{n\ell}(r)$ and level fit.
